...
def main() -> None:
    ...
    for required_phrase in [
        ...
        "saved Codex project",
        "expansion queue",
        "Budget joins separately from worker handoffs",
        "combined validation payload",
        "JOIN_MANIFEST_MAX_CHARS",
        "baseline tier",
        "escalation cap",
    ...
    for required_phrase in [
        ...
        "maxOutputCharsPerItem",
        "expansion_queue",
        "worker limit is not a combined payload limit",
        "compact join manifest",
        "resumable handles",
        "same-model retry",
        ...
        "`not_started`",
        "collection-attempt budget expires",
        "MIN_ACTIVE_POLL_DELAY_MS",
        "wall-clock deadline",
    ]:
    ...
    for required_phrase in [
        ...
        "combined join as one worker handoff",
    ...
    for required_phrase in [
        ...
        "fenced JSON verdict block",
        "same trigger set",
        "per-record size input",
        "sample record",
        "allocated budget",
        "escalation gates from that count",
    ]:
